...
export function createXlsxSheetViewQuery(
  ...
  return `CREATE OR REPLACE VIEW main.${toDuckDBIdentifier(viewName)} AS SELECT * FROM read_xlsx(${quote(fileName, { single: true })}, sheet=${quote(sheetName, { single: true })}, ignore_errors=true);`;
}
