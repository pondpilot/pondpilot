...
function sanitizeErrorMessage(message: string): string {
  return (
    message
      ...
      .replace(/\/[\w/.-]+\.(js|ts|tsx|jsx)/gi, '[path]')
      // Remove stack traces
...
export class AIService {
  ...
  async testConnection(): Promise<{ success: boolean; message: string }> {
    ...
    try {
      ...
      if (response.success) {
        const providerName = isPollyProvider(this.config.provider)
          ? POLLY_CONFIG.DISPLAY_NAME
          : this.config.provider === PROVIDER_IDS.CUSTOM
            ? 'custom endpoint'
