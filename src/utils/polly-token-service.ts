...
/**
 * Get the current token's rate limit info (if available)
 */
export function getDemoTokenLimits(): { requestsPerHour: number } | null {
  if (cachedToken) {
    return cachedToken.limits;
  }

  const storedToken = loadTokenFromStorage();
  if (storedToken) {
    return storedToken.limits;
  }
 ...
 */
export function hasCachedDemoToken(): boolean {
  if (cachedToken && isTokenValid(cachedToken)) {
    return true;
  }
