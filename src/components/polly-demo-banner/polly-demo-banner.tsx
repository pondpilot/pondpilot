...
/**
 * Check if the Polly demo banner should be shown
 */
export function shouldShowPollyDemoBanner(): boolean {
  const config = getAIConfig();
  if (!isPollyProvider(config.provider)) {
    return false;
  }
...
/**
 * Reset the Polly demo banner dismissed state (for testing)
 */
