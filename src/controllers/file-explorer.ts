...
export const renameFile = async (
  ...
  if (!dataSource || dataSource.type === 'attached-db' || dataSource.type === 'remote-db' || dataSource.type === 'httpserver-db') {
...
export const renameXlsxFile = async (
  ...
  const curDataSources = [...dataSources.values()].filter(
    (ds) =>
      ds.type !== 'attached-db' && ds.type !== 'remote-db' && ds.type !== 'httpserver-db' && ds.fileSourceId === localEntryId,
  );
