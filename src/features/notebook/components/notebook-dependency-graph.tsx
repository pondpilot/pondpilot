...
function getStatusColor(status: CellExecutionState['status'], isDarkMode: boolean): string {
  switch (status) {
    case 'running':
    ...
    const edges = useMemo((): Edge<NotebookDependencyEdgeData>[] => {
      return baseEdges.map((edge) => {
        ...
        const isDirectlyConnected =
          selectedCellId === edge.source || selectedCellId === edge.target;
        ...
        >
          <Panel position="top-left">
            ...
            >
              <Tooltip
                label={
                  direction === 'LR' ? 'Switch to vertical layout' : 'Switch to horizontal layout'
                }
              >
                <ActionIcon
                  size="sm"
