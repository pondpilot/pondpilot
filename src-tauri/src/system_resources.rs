...
pub fn calculate_resource_limits() -> ResourceLimits {
    ...
    let primary_memory_mb = std::cmp::min((total_memory_mb as f64 * 0.25) as usize, 8192);
    let primary_threads = std::cmp::max((cpu_count as f64 * 0.50) as usize, 2);

    // Pool connections get 10% of memory (max 2GB) and 25% of CPU cores
    let pool_memory_mb = std::cmp::min((total_memory_mb as f64 * 0.10) as usize, 2048);
    let pool_threads = std::cmp::max((cpu_count as f64 * 0.25) as usize, 1);
