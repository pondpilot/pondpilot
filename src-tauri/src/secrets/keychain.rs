...
use super::models::{SecretCredentials, SecretFields, SecretMetadata, SecretType, SecureString};
...
impl KeychainProvider for NativeKeychainProvider {
    async fn store_secret(
        ...
        tracing::debug!(
            "[Keychain] Storing secret: Name={}, Type={:?}",
            metadata.name,
        ...
        tracing::debug!(
            "[Keychain] Prepared {} credential fields",
            credentials.len()
        );
        ...
        tracing::debug!(
            "[Keychain] Serialized payload length: {} bytes",
            payload.len()
        );

        let _entry = Entry::new(&self.service_name, &account).map_err(|e| {
    ...
    async fn get_secret(&self, secret_id: &Uuid) -> Result<SecretCredentials, SecretError> {
        ...
        tracing::debug!(
            "[Keychain] Retrieved metadata for secret: {} (type: {:?})",
            metadata.name,
            metadata.secret_type
        );
        ...
        tracing::debug!(
            "[Keychain] Successfully retrieved {} credential fields",
            credentials.len()
        );
    ...
    async fn delete_secret(&self, secret_id: &Uuid) -> Result<(), SecretError> {
        ...
        if let Ok(metadata) = metadata_result {
            let account = self.build_account_name(&metadata.secret_type, secret_id);
            tracing::debug!(
                "[Keychain] Attempting to delete keychain entry: {}",
                account
            );
            ...
            match delete_result {
                ...
                Ok(Err(keyring::Error::NoEntry)) => {
                    tracing::debug!(
                        "[Keychain] Keychain entry not found (already deleted or never created)"
                    );
                }
                Ok(Err(e)) => {
...
impl KeychainProvider for DisabledKeychainProvider {
    async fn store_secret(
        ...
        Err(SecretError::KeychainError(
            "Secrets manager is disabled - keychain unavailable".to_string(),
        ))
    }

    async fn get_secret(&self, _secret_id: &Uuid) -> Result<SecretCredentials, SecretError> {
        Err(SecretError::KeychainError(
            "Secrets manager is disabled - keychain unavailable".to_string(),
        ))
    }
    ...
    async fn delete_secret(&self, _secret_id: &Uuid) -> Result<(), SecretError> {
        Err(SecretError::KeychainError(
            "Secrets manager is disabled - keychain unavailable".to_string(),
        ))
    }

    async fn update_last_used(&self, _secret_id: &Uuid) -> Result<(), SecretError> {
        Err(SecretError::KeychainError(
            "Secrets manager is disabled - keychain unavailable".to_string(),
        ))
    }
