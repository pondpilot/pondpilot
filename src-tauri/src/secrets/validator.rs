use super::models::{SecretType, SecretFields, SecretCredentials};
...
impl SecretValidator {
    ...
    pub fn validate_fields(&self, secret_type: &SecretType, fields: &SecretFields) -> Result<(), SecretError> {
        match secret_type {
            ...
            SecretType::HTTP => {
                ...
                if fields.bearer_token.is_none() && 
                   (fields.basic_username.is_none() || fields.basic_password.is_none()) {
                    return Err(SecretError::MissingCredential("bearer_token or basic auth credentials".to_string()));
                }
            },
        }
