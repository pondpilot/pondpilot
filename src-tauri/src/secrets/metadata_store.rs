...
impl MetadataStore {
    ...
    fn get_db_path() -> Result<PathBuf, SecretError> {
        let data_dir = dirs::data_dir().ok_or_else(|| {
            SecretError::DatabaseError("Failed to get data directory".to_string())
        })?;
        let app_dir = data_dir.join("pondpilot");
        std::fs::create_dir_all(&app_dir).map_err(|e| SecretError::DatabaseError(e.to_string()))?;
    ...
    pub async fn get_metadata(&self, secret_id: &Uuid) -> Result<SecretMetadata, SecretError> {
        ...
        let metadata = stmt
            .query_row(params![secret_id.to_string()], |row| {
                ...
                let secret_type = SecretType::from_string(&secret_type_str).ok_or_else(|| {
                    rusqlite::Error::FromSqlConversionFailure(
                        2,
                        rusqlite::types::Type::Text,
            ...
            .map_err(|e| {
                eprintln!(
                    "[MetadataStore] Failed to get metadata for secret {}: {:?}",
                    secret_id, e
                );
