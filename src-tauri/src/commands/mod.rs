...
#[tauri::command]
pub async fn initialize_duckdb(engine: EngineState<'_>, config: EngineConfig) -> Result<()> {
...
#[tauri::command]
pub async fn get_tables(engine: EngineState<'_>, database: String) -> Result<Vec<TableInfo>> {
    // No lock needed - read-only operation
...
#[tauri::command]
pub async fn register_file(engine: EngineState<'_>, options: FileRegistration) -> Result<()> {
...
pub async fn create_connection(engine: EngineState<'_>) -> Result<String> {
    ...
    eprintln!(
        "[CREATE_CONNECTION] Created persistent connection: {}",
        connection_id
    );
    Ok(connection_id)
}
...
pub async fn connection_execute(
    ...
    if sql.trim_start().to_uppercase().starts_with("ATTACH ") {
        ...
        let (quote, start_idx) = if let Some(idx) = sql.find('\'') {
            ('\'', idx)
        } else if let Some(idx) = sql.find('"') {
            ('"', idx)
        } else {
            ('\'', usize::MAX)
        };
...
#[tauri::command]
pub async fn connection_close(engine: EngineState<'_>, connection_id: String) -> Result<()> {
...
#[tauri::command]
pub async fn reset_all_connections(engine: EngineState<'_>) -> Result<()> {
    #[cfg(debug_assertions)]
...
#[tauri::command]
pub async fn prepare_statement(engine: EngineState<'_>, sql: String) -> Result<String> {
...
pub async fn prepared_statement_execute(
    ...
    let statement_id = crate::commands::utils::coalesce_param_opt(
        statement_id,
        statementId,
        "statement_id",
    ...
    engine
        .execute_prepared_statement(&statement_id, params)
        .await
}
...
pub async fn list_extensions(engine: EngineState<'_>) -> Result<Vec<ExtensionInfo>> {
    let sql =
        "SELECT extension_name, loaded, installed, extension_version FROM duckdb_extensions()";
