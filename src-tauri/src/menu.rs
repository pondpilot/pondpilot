...
pub fn create_menu(app: &AppHandle) -> Result<Menu<tauri::Wry>, Box<dyn std::error::Error>> {
    ...
    #[cfg(target_os = "macos")]
    {
        let app_menu = SubmenuBuilder::new(app, "PondPilot")
            .item(&PredefinedMenuItem::about(
                app,
                Some("About PondPilot"),
                None,
            )?)
            .separator()
            .item(
                &MenuItemBuilder::with_id("secrets", "Secrets Manager")
    ...
    let file_menu = SubmenuBuilder::new(app, "File")
        ...
        .separator()
        .item(
            &MenuItemBuilder::with_id("export", "Export Data...")
    ...
    let database_menu = SubmenuBuilder::new(app, "Database")
        .item(
            &MenuItemBuilder::with_id("connect", "Connect to Database...")
                .accelerator("Cmd+Shift+C")
                .build(app)?,
        )
        .item(&MenuItemBuilder::with_id("disconnect", "Disconnect").build(app)?)
        .separator()
        .item(
            &MenuItemBuilder::with_id("execute_query", "Execute Query")
                .accelerator("Cmd+Enter")
                .build(app)?,
        )
        .item(
            &MenuItemBuilder::with_id("cancel_query", "Cancel Query")
                .accelerator("Escape")
                .build(app)?,
        )
        .separator()
        .item(
            &MenuItemBuilder::with_id("refresh_schema", "Refresh Schema")
    ...
    let window_menu = SubmenuBuilder::new(app, "Window")
        ...
        .separator()
        .item(
            &MenuItemBuilder::with_id("main_window", "Main Window")
                .accelerator("Cmd+1")
                .build(app)?,
        )
        .item(
            &MenuItemBuilder::with_id("secrets_window", "Secrets Manager")
                .accelerator("Cmd+2")
                .build(app)?,
        )
        .separator()
    ...
    let help_menu = SubmenuBuilder::new(app, "Help")
        ...
        .separator()
        .item(&MenuItemBuilder::with_id("about", "About PondPilot").build(app)?)
        .build()?;
