...
impl StreamManager {
    ...
    pub async fn register_stream(
        &self,
        stream_id: String,
    ) -> (CancellationToken, mpsc::Receiver<()>) {
        ...
        tracing::debug!(
            "[StreamManager] Stream {} registered, active streams: {}",
            stream_id,
            streams.len()
        );
    ...
    pub async fn acknowledge_batch(&self, stream_id: &str) -> Result<()> {
        ...
        if let Some(handle) = streams.get(stream_id) {
            match handle.ack_sender.send(()).await {
                Ok(_) => {
                    tracing::trace!(
                        "[StreamManager] Batch acknowledged for stream {}",
                        stream_id
                    );
                }
                Err(_e) => {
    ...
    pub async fn cancel_stream(&self, stream_id: &str) -> Result<()> {
        ...
        if let Some(handle) = streams.remove(stream_id) {
            tracing::debug!(
                "[StreamManager] Found stream {} handle, triggering cancellation token",
                stream_id
            );
            handle.cancel_token.cancel();
            tracing::debug!(
                "[StreamManager] Cancellation token triggered for stream {}",
                stream_id
            );
        } else {
            tracing::warn!(
                "[StreamManager] Stream {} not found in active streams!",
                stream_id
            );
        }
        tracing::debug!(
            "[StreamManager] Active streams after cancel: {}",
            streams.len()
        );
        Ok(())
    }
    ...
    pub async fn cleanup_stream(&self, stream_id: &str) {
        ...
        if streams.remove(stream_id).is_some() {
            tracing::debug!(
                "[StreamManager] Stream {} removed from active streams",
                stream_id
            );
        } else {
        ...
        tracing::debug!(
            "[StreamManager] Active streams after cleanup: {}",
            streams.len()
        );
    }
}
