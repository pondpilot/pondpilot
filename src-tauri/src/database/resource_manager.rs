...
impl Drop for ResourceGuard {
    fn drop(&mut self) {
        tracing::debug!(
            "[RESOURCE_MANAGER] Releasing resources for query {}",
            self.query_id
        );
        ...
        tokio::spawn(async move {
            ...
            if let Some(metrics) = active.remove(&query_id) {
                ...
                tracing::debug!(
                    "[RESOURCE_MANAGER] Released {}MB for query {}",
                    used / 1024 / 1024,
                    query_id
                );
            }
...
impl ResourceManager {
    pub fn new(total_memory: usize, total_connections: usize) -> Self {
        ...
        tracing::info!(
            "[RESOURCE_MANAGER] Initialized with {}MB memory ({} permits)",
    ...
    pub async fn acquire_for_query(
        ...
        let estimated_memory = hints.memory_limit.unwrap_or(self.default_query_memory());

        tracing::debug!(
            "[RESOURCE_MANAGER] Query {} requesting {}MB",
            query_id,
        ...
        self.active_queries
            .write()
            .await
            .insert(query_id.clone(), metrics);
        self.used_memory
            .fetch_add(estimated_memory, Ordering::Relaxed);

        tracing::debug!(
            "[RESOURCE_MANAGER] Granted {}MB to query {} ({}MB total in use)",
            estimated_memory / 1024 / 1024,
            query_id,
            self.used_memory.load(Ordering::Relaxed) / 1024 / 1024
        );
