...
- [x] Define the `.sqlnb` JSON format specification:
  ```json
  {
    "version": 1,
    "name": "Notebook Name",
...
- [x] Register `.sqlnb` as a known file type in the app's file handling logic (so the file explorer recognizes it)
- [x] Verify: export a notebook as `.sqlnb`, re-import it — all cells and content preserved. Export as HTML, open in browser — readable document with SQL, markdown, and result tables.
